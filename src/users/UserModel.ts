...
import { userModel as db, IUser } from './UserSchema';
...
export let registerUser = (user: IUser, password: string): Promise<User> => {
...
export let authenticateUser = (params: AuthData): Promise<User> => {
...
export let updateUser = (id: string, user: AuthData, updates: any): Promise<updateNotify> => {
    /**
     * @docs: 
...
export let getOneUser = (id: string): Promise<IUser> => {
    let _id = <mongoose.Types.ObjectId>mongoose.Types.ObjectId(id);
    return db
        .findById(_id)
        .select({ 'password': 0 })
